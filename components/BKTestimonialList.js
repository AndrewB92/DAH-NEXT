...
import BKTestimonialRender from './BKTestimonialRender';

const BKTestimonialList = () => {
    const [reviews, setReviews] = useState([]);
    const [currentIndex, setCurrentIndex] = useState(0); // track the current slide
    const sliderRef = useRef(null); // Reference for slider wrapper
    ...
    // Function to handle moving to the previous review
    const handlePrev = () => {
        setCurrentIndex((prevIndex) => Math.max(prevIndex - 1, 0));
    };

    // Function to handle moving to the next review
    const handleNext = () => {
        setCurrentIndex((prevIndex) => Math.min(prevIndex + 1, reviews.length - 1));
    };

    // Calculate the translateX value based on the current index, centering the active review
    const sliderTransform = `translateX(calc(${50 - currentIndex * (100 / reviews.length)}%))`;

    return (
        <>
            <div className="cont cont-row gap-30 testimonial--container slider">
                <div
                    id="slider-wrapper"
                    className="slider-wrapper"
                    ref={sliderRef}
                    style={{ transform: sliderTransform, transition: 'transform 0.5s ease' }}
                >
                    {reviews.map((review, index) => (
                        <div key={index} style={{ flex: '0 0 30%' }}>
                            <BKTestimonialRender review={review} />
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="cont cont-row buttons-wrapper">
                ...
                <button id="next" onClick={handleNext} disabled={currentIndex === reviews.length - 1}>
                    <svg width="9" height="16" viewBox="0 0 9 16" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className="cont cont-col progress-container" aria-label="Progress Indicator">
                <div className="cont cont-row gap-30 progress-steps" role="progressbar" aria-valuemin="1" aria-valuemax={reviews.length} aria-valuenow={currentIndex + 1}>
                    {reviews.map((_, index) => (
                        <div key={index} className={`progress-step dot ${index === currentIndex ? 'active' : ''}`}></div>
                    ))}
                </div>
            </div>
        </>
...
export default BKTestimonialList;