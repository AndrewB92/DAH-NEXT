import React from 'react';
import { useState, useEffect} from 'react'
import styles from '../styles/DatePickerForm.module.css'
import { useRouter } from 'next/router';
import DatePicker from 'react-datepicker'
import datetime from 'date-and-time'
import Container from 'react-bootstrap/Container'
import Row from 'react-bootstrap/Row'
import Col from 'react-bootstrap/Col'
import Cookies from 'js-cookie'
import 'react-datepicker/dist/react-datepicker.css'
import rootUrl from 'config.js'

const BKDatePicker = ( {fromDateState, toDateState, areaState, roomsState} ) => {

    const router = useRouter();

    function formatDateForGET(date) {

        const year = date.getUTCFullYear()
        const month = String(date.getUTCMonth() + 1).padStart(2, '0')
        const day = String(date.getUTCDate()).padStart(2, '0')
      
        return `${year}-${month}-${day}`
    }
    
    const handleSearch = () => {
        const checkInDate = formatDateForGET(startDate)
        const checkOutDate = formatDateForGET(endDate)
        const location = "Dublin"
        const url = `/availability/${checkInDate}/${checkOutDate}/${rooms}/${location}`;
        router.push(url);
    };

    const [startDate, setStartDate] = useState(fromDateState ? new Date(fromDateState) : new Date());
    const [endDate, setEndDate] = useState(toDateState ? new Date(toDateState) : new Date());
    const [rooms, setRooms] = useState(roomsState);

    const roomDropDown = 'rooms';

    const onStartChange = (date) => {
        setStartDate(date);
        setEndDate(date)
    }

    const onEndChange = (date) => {
        setEndDate(date);
    }

    const formatDate = (date) => {
        const months = [
            'January', 'February', 'March', 'April', 'May', 'June',
            'July', 'August', 'September', 'October', 'November', 'December'
        ];
        const day = date.getDate();
        const month = months[date.getMonth()];
        const year = date.getFullYear();
    
        return `${month} ${day}, ${year}`;
    };

    return (
        <div className="datePickerCol">
            <Row>
                <Col md={12} lg={3} className={styles.fieldFirst}>
                    <span className={styles.fieldLabel}>Check-in date</span>
                    <DatePicker
                        selected={startDate}
                        onChange={(date) => onStartChange(date)}
                        dateFormat="MMMM d, yyyy"
                        minDate={new Date()}
                        customInput={
                            <button
                                className={styles.datePickerInput}
                                style={{
                                    textAlign: 'left'
                                }}
                            >
                                {startDate ? formatDate(startDate) : 'Select a date'}
                            </button>
                        }
                    />
                </Col>
                <Col md={12} lg={3} className={styles.field}>
                <span className={styles.fieldLabel}>Check-out date</span>
                    <DatePicker
                        selected={endDate}
                        onChange={(date) => onEndChange(date)}
                        dateFormat="MMMM d, yyyy"
                        minDate={startDate}
                        customInput={
                            <button
                                className={styles.datePickerInput}
                                style={{
                                    textAlign: 'left'
                                }}
                            >
                                {startDate ? formatDate(endDate) : 'Select a date'}
                            </button>
                        }
                    />
                </Col>
                <Col md={12} lg={3} className={styles.field}>
                    <span className={styles.fieldLabel}>Rooms</span>
                    <span>
                        <select
                            name="first"
                            value={rooms}
                            onChange={(e) => {
                                setRooms(e.target.value)
                            }}
                        >
                            <option value="1">1 Bed or more</option>
                            <option value="2">2 Beds or more</option>
                            <option value="3">3 Beds or more</option>
                            <option value="4">Any Beds</option>
                        </select>
                    </span>
                </Col>
                <Col md={12} lg={3}  className={styles.field}>
                    <button className={styles.searchButton} onClick={handleSearch}>
                        <span className={styles.searchButtonText}>Search
                        <svg width="33" height="32" viewBox="0 0 33 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M11.6383 21.5096L10.9901 20.8614C10.8111 20.6824 10.8111 20.3922 10.9901 20.2132L18.8785 12.3249L13.1162 12.3314C12.994 12.3319 12.8766 12.2835 12.7902 12.1971C12.7038 12.1107 12.6555 11.9934 12.656 11.8711L12.6624 10.9572C12.6619 10.835 12.7103 10.7177 12.7967 10.6312C12.8831 10.5448 13.0004 10.4965 13.1227 10.497L21.0758 10.4905C21.2581 10.4904 21.433 10.5627 21.562 10.6915L21.8083 10.9378C21.9354 11.0679 22.0074 11.242 22.0092 11.4239L22.0027 19.3771C22.0032 19.4993 21.9549 19.6166 21.8685 19.7031C21.7821 19.7895 21.6647 19.8378 21.5425 19.8373L20.6221 19.8373C20.5014 19.8385 20.3853 19.7911 20.2999 19.7058C20.2146 19.6204 20.1672 19.5043 20.1684 19.3836L20.1749 13.6212L12.2865 21.5096C12.1075 21.6886 11.8173 21.6886 11.6383 21.5096Z" fill="white"/>
                        </svg>
                        </span>
                    </button>
                </Col>
            </Row>
            <Row className={styles.link}>
                <a href={`${rootUrl}/make-an-enquiry`}>Short on time? click here to use our enquiry form instead</a>
            </Row>
        </div>
    );
};

export default BKDatePicker;